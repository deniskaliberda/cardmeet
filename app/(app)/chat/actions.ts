...
type DMRow = {
  ...
  read: boolean;
  created_at: string;
  sender: ProfileMini | null;
  receiver: ProfileMini | null;
};
...
export async function getDMConversations() {
  ...
  const messages = (rawMessages ?? []) as unknown as DMRow[];
  ...
  for (const msg of messages) {
    ...
    seen.add(partnerId);
    const partner = msg.sender_id === user.id ? msg.sender : msg.receiver;
    if (!partner) continue;
    conversations.push({
...
export async function getDMMessages(friendId: string) {
  ...
  if (!parsed.success) return { messages: [] as DMMessage[] };
  const safeFriendId = parsed.data;

  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return { messages: [] as DMMessage[] };
  ...
  return { messages: (data ?? []) as unknown as DMMessage[] };
}
